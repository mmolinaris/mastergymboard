        ...
        <div style={{ padding: "0 16px 16px" }}>
          ...
          <div style={{ background: "#222", borderRadius: 10, padding: "10px 12px", marginBottom: 10 }}>
            ...
            <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
              <input value={weightInput} onChange={e => setWeightInput(e.target.value)} placeholder="kg" type="number" inputMode="decimal"
                style={{ flex: 1, background: "#333", border: "1px solid #444", borderRadius: 8, padding: "8px 12px", color: "#FFF", fontSize: "16px", fontWeight: 700, outline: "none" }} />
...
function ErrorScreen({ error, onRetry }) {
  return (
    <div style={{ minHeight: "100vh", background: "#0A0A0A", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: 24, gap: 16 }}>
      ...
      <p style={{ color: "#555", fontSize: "11px", textAlign: "center", maxWidth: 300 }}>
        Verifica che il foglio Google Sheet sia pubblico (Chiunque abbia il link) e che la API Key sia corretta.
      </p>
      <button onClick={onRetry} style={{ background: "#FF6B00", border: "none", borderRadius: 12, padding: "14px 32px", color: "#FFF", fontWeight: 800, fontSize: "16px", cursor: "pointer", marginTop: 8 }}>
        Riprova
      </button>
    </div>
  );
...
function LoginScreen({ config, styles, onLogin, error }) {
  ...
  return (
    <div style={{ minHeight: "100vh", background: styles.bg, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "32px 24px" }}>
      <div style={{ width: 80, height: 80, borderRadius: 20, background: `${styles.primary}22`, display: "flex", alignItems: "center", justifyContent: "center", marginBottom: 20, border: `2px solid ${styles.primary}44` }}>
        {config.logo_url && config.logo_url !== "https://link-al-logo.png" ? (
          <img src={config.logo_url} alt="logo" style={{ width: 60, height: 60, objectFit: "contain", borderRadius: 12 }} />
        ) : (
  ...
  return (
    <div style={{ padding: "20px 16px 100px", minHeight: "100vh", background: styles.bg }}>
      ...
      <div style={{ background: `linear-gradient(135deg, ${styles.primary}22, ${styles.primary}08)`, borderRadius: 20, padding: "20px", marginBottom: 24, border: `1px solid ${styles.primary}33` }}>
        ...
        {scheda.note_trainer && (
          <div style={{ marginTop: 12, fontSize: "12px", color: styles.textSecondary, fontStyle: "italic", borderTop: `1px solid ${styles.primary}22`, paddingTop: 10 }}>
            💡 {scheda.note_trainer}
          </div>
        )}
      </div>
      ...
      {giorni.map((g, i) => (
        <button key={g.nome} onClick={() => onSelectDay(g.nome)}
          style={{ width: "100%", background: styles.card, border: `1px solid ${styles.cardBorder}`, borderRadius: 14, padding: "16px", marginBottom: 10, cursor: "pointer", display: "flex", alignItems: "center", gap: "12px", textAlign: "left" }}>
          ...
          <div key={group} style={{ marginBottom: 20 }}>
            ...
            {exs.map((ex, i) => (
              <ExerciseCard key={i} ex={ex} styles={styles} onTimer={onTimer} onVideo={onVideo}
                progress={progressData[`${ex.esercizio}__${ex.scheda_id}`] || []} onLogWeight={onLogWeight} />
            ))}
          </div>
        ))}
      </div>
...
function ProgressTracker({ progressData, styles, esercizi, schedaAttiva }) {
  ...
  const activeExercises = useMemo(() => {
    return [...new Set(esercizi.filter(e => e.scheda_id === schedaAttiva).map(e => e.esercizio))];
      ...
      <div style={{ display: "flex", flexWrap: "wrap", gap: 8, marginBottom: 16 }}>
        {activeExercises.map(ex => (
          <button key={ex} onClick={() => setSelectedEx(selectedEx === ex ? null : ex)}
            style={{ background: selectedEx === ex ? styles.primary : "#222", border: `1px solid ${selectedEx === ex ? styles.primary : styles.cardBorder}`, borderRadius: 8, padding: "6px 12px", cursor: "pointer", color: selectedEx === ex ? "#FFF" : styles.textSecondary, fontSize: "12px", fontWeight: 600 }}>
            {ex}
          </button>
        ))}
      </div>
  ...
  return (
    <div style={{ maxWidth: 480, margin: "0 auto", position: "relative", background: styles.bg, minHeight: "100vh" }}>
      ...
      <div style={{ animation: "fadeIn 0.3s ease" }}>
        {selectedDay ? (
          <WorkoutDay giorno={selectedDay} esercizi={dayExercises} styles={styles} onBack={() => setSelectedDay(null)}
            onTimer={s => setTimerSeconds(s)} onVideo={v => setVideoUrl(v)} progressData={progressData} onLogWeight={handleLogWeight} />
        ) : activeTab === "home" && currentScheda ? (
          <Dashboard cliente={currentCliente} scheda={currentScheda} esercizi={appData.esercizi} styles={styles}
            onSelectDay={day => setSelectedDay(day)} onSync={loadData} lastSync={lastSync} />
        ) : activeTab === "progress" ? (
