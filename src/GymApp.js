...
async function fetchAllData() {
  ...
  // Normalizza esercizi: supporta sia "seduta" che "giorno", sia "recupero" che "riposo_sec"
...
// TEMA: sfondo bianco, testo nero, accento rosso
const ST = (p, b) => ({
  bg: "#FFFFFF",
...
function Dashboard({cliente,scheda,esercizi,s,onSelectDay,onSync,lastSync}){
  ...
  return(
    <div style={{padding:"20px 16px 100px",minHeight:"100vh",background:s.bg}}>
      ...
      {sedute.length===0&&<p style={{color:s.sub,fontSize:14}}>Nessuna seduta trovata per questa scheda.</p>}
      {sedute.map((sed,i)=>(
      ...
      <button onClick={onSync} style={{width:"100%",background:s.card,border:`1px solid ${s.border}`,borderRadius:12,padding:12,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",gap:8,color:s.sub,fontSize:13,marginTop:8}}><RefreshCw size={14}/> Aggiorna scheda {lastSync&&`• ${lastSync}`}</button>
    </div>);
}
...
function WorkoutDay({seduta,esercizi,s,onBack,onTimer,onVideo,progressData,onLog}){
  ...
  return(
    <div style={{padding:"0 16px 100px",minHeight:"100vh",background:s.bg}}>
      <div style={{position:"sticky",top:0,zIndex:50,background:s.bg,padding:"28px 0 12px",display:"flex",alignItems:"center",gap:12,borderBottom:`1px solid ${s.border}`}}>
    ...
    <div style={{padding:"20px 16px 100px",minHeight:"100vh",background:s.bg}}>
      ...
      return(<div key={sc.scheda_id} style={{background:s.card,borderRadius:16,marginBottom:12,border:`1px solid ${active?s.primary+"44":s.border}`,overflow:"hidden"}}>
        ...
        {isOpen&&<div style={{padding:"12px 16px 16px",borderTop:`1px solid ${s.border}`}}>
          ...
          {seds.map(sed=>{const dayExs=exs.filter(e=>e.seduta===sed).sort((a,b)=>parseInt(a.ordine||0)-parseInt(b.ordine||0));return(<div key={sed} style={{marginBottom:12}}><div style={{fontSize:12,fontWeight:700,color:s.primary,marginBottom:6}}>{sed} {dayExs[0]?.tipo_seduta?`— ${dayExs[0].tipo_seduta}`:""}</div>{dayExs.map((e,i)=><div key={i} style={{display:"flex",justifyContent:"space-between",padding:"6px 0",borderBottom:`1px solid ${s.border}`}}><span style={{color:s.text,fontSize:13}}>{e.esercizio}</span><span style={{color:s.sub,fontSize:13}}>{isCardio(e)?e.ripetizioni:`${e.serie}×${e.ripetizioni}`}</span></div>)}</div>)})}
        </div>}
      </div>)})}
    </div>);
}
  ...
  return(
    <div style={{width:"100%",margin:"0 auto",position:"relative",background:"#FFFFFF",minHeight:"100vh"}}>
      ...
      <div>
        ...
        :tab==="home"&&!scheda?<div style={{padding:"40px 24px",textAlign:"center"}}><AlertCircle size={40} color="#E53935" style={{margin:"0 auto 16px"}}/><h2 style={{color:"#1A1A1A",fontSize:18,fontWeight:800,marginBottom:8}}>Scheda non trovata</h2><p style={{color:"#666",fontSize:14}}>Contatta il tuo trainer per assegnare una scheda.</p></div>
