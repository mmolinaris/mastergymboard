...
function ExCard({ex,s,onTimer,onVideo,progress,onLog,ultimoPeso}){
  ...
  return(
    <div style={{background:s.card,borderRadius:16,overflow:"hidden",border:`1px solid ${s.border}`,marginBottom:12}}>
      ...
      {open&&(<div style={{padding:"0 16px 16px"}}>
        ...
        {/* CAMPO PESO — con ultima volta */}
        <div style={{background:"#EBEBEB",borderRadius:10,padding:"10px 12px",marginBottom:10}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:6}}>
            <div style={{fontSize:12,color:s.sub}}>IL TUO PESO OGGI</div>
            {ultimoPeso && <div style={{fontSize:11,color:s.primary,fontWeight:700}}>↩ Ultima volta: {ultimoPeso.peso_kg} kg · {ultimoPeso.data.slice(0,5)}</div>}
          </div>
...
function WorkoutDay({seduta,esercizi,s,onBack,onTimer,onVideo,progressData,onLog,ultimiPesi}){
  ...
  return(
    <div style={{padding:"0 16px 100px",minHeight:"100vh",background:s.bg}}>
      ...
      <div style={{paddingTop:16}}>{[...esercizi].sort((a,b)=>parseInt(a.ordine||0)-parseInt(b.ordine||0)).map((ex,i)=><ExCard key={i} ex={ex} s={s} onTimer={onTimer} onVideo={onVideo} progress={progressData[`${ex.esercizio}__${ex.scheda_id}`]||[]} onLog={onLog} ultimoPeso={ultimiPesi?.[ex.esercizio]}/>)}</div>
    </div>);
}
...
export default function GymApp(){
  ...
  const [ultimiPesi, setUltimiPesi]=useState({});
  ...
  const handleLogout=useCallback(()=>{setLoggedIn(false);setCliente(null);setTab("home");setSelDay(null);setProgress({});setUltimiPesi({});try{localStorage.removeItem("gb_code");localStorage.removeItem("gb_pin")}catch(e){}},[]);
