...
import {
  ...
  TrendingUp, Video, AlertCircle, Zap, Clock, Award
} from "lucide-react";
...
// Palette ispirata al logo: grigio, bianco, nero, rosso
const S = {
  bg: "#F4F4F4",
  white: "#FFFFFF",
  card: "#FFFFFF",
...
const fmtDate = d => {
  if (!d) return "";
  if (d.includes("/")) { const p=d.split("/"); return p.length===3?`${p[0].padStart(2,"0")}/${p[1].padStart(2,"0")}/${p[2]}`:d; }
  const p = d.split("-");
...
function Nav({ tab, onNav }) {
  return (
    <div style={{position:"fixed",bottom:0,left:0,right:0,zIndex:100,background:S.navBg,borderTop:`1px solid ${S.border}`,display:"flex",justifyContent:"space-around",padding:"8px 0 env(safe-area-inset-bottom,10px)",boxShadow:"0 -2px 12px rgba(0,0,0,0.06)"}}>
      {[["home",Home,"Home"],["progress",BarChart3,"Progressi"],["history",ClipboardList,"Storico"],["profile",User,"Profilo"]].map(([id,Icon,lbl]) => {
        ...
        return (
          <button key={id} onClick={()=>onNav(id)} style={{background:"none",border:"none",cursor:"pointer",display:"flex",flexDirection:"column",alignItems:"center",gap:3,color:a?S.red:S.muted,padding:"4px 16px",minWidth:60}}>
            ...
            <span style={{fontSize:10,fontWeight:a?700:500,letterSpacing:"0.3px"}}>{lbl}</span>
          </button>
        );
      })}
    </div>
...
function RestTimer({ seconds, onClose }) {
  ...
  useEffect(()=>{if(run&&rem>0){ref.current=setInterval(()=>setRem(r=>{if(r<=1){clearInterval(ref.current);try{navigator.vibrate?.([300,100,300,100,300])}catch(e){}return 0}return r-1}),1000)}return()=>clearInterval(ref.current)},[run,rem]);
    ...
    <div style={{position:"fixed",inset:0,zIndex:200,background:"rgba(20,20,20,0.97)",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:32,padding:24}}>
      ...
      <div style={{position:"relative",width:200,height:200}}>
        <svg width="200" height="200" viewBox="0 0 200 200" style={{transform:"rotate(-90deg)"}}>
          ...
          <circle cx="100" cy="100" r={R} fill="none" stroke={done?"#4CAF50":S.red} strokeWidth="6" strokeLinecap="round" strokeDasharray={C} strokeDashoffset={off} style={{transition:"stroke-dashoffset 1s linear"}}/>
        </svg>
        <div style={{position:"absolute",inset:0,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center"}}>
          ...
          {done&&<span style={{fontSize:13,color:"#4CAF50",fontWeight:700,marginTop:4}}>RIPRENDI!</span>}
        </div>
      </div>
...
function VideoModal({url,onClose}){
  ...
  return(
    <div style={{position:"fixed",inset:0,zIndex:200,background:"rgba(0,0,0,0.97)",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center"}}>
      <button onClick={onClose} style={{position:"absolute",top:"max(16px, env(safe-area-inset-top, 16px))",right:16,background:"#333",border:"none",borderRadius:"50%",width:40,height:40,color:"#FFF",cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",zIndex:10}}><X size={20}/></button>
...
function ExCard({ex,onTimer,onVideo,progress,onLog}){
  ...
  if(cardio) return(
    <div style={{background:S.card,borderRadius:12,border:`1px solid ${S.border}`,marginBottom:8,padding:"12px 16px",display:"flex",alignItems:"center",gap:12,boxShadow:"0 1px 4px rgba(0,0,0,0.05)"}}>
      ...
      <div style={{flex:1}}><div style={{fontSize:14,fontWeight:700,color:S.text}}>{ex.esercizio}</div>{hasNote&&<div style={{fontSize:12,color:S.sub,marginTop:1}}>{ex.note}</div>}</div>
    ...
    <div style={{background:S.card,borderRadius:14,overflow:"hidden",border:`1px solid ${S.border}`,marginBottom:10,boxShadow:"0 1px 6px rgba(0,0,0,0.05)"}}>
      ...
      {open&&(
        <div style={{padding:"0 16px 16px",borderTop:`1px solid ${S.border}`}}>
          ...
          <div style={{display:"flex",gap:8,margin:"12px 0"}}>
            {[ex.serie&&["SERIE",ex.serie],ex.ripetizioni&&["REPS",ex.ripetizioni],["REC.",ex.recupero&&ex.recupero!=="0"?ex.recupero+"s":"—"]].filter(Boolean).map(([l,v])=>(
              <div key={l} style={{background:S.grayLight,borderRadius:10,padding:"8px 12px",flex:1,textAlign:"center"}}>
                <div style={{fontSize:10,color:S.muted,marginBottom:2,fontWeight:600,letterSpacing:"0.5px"}}>{l}</div>
          ...
          {/* Peso suggerito */}
          {hasPeso&&<div style={{background:S.redLight,borderRadius:10,padding:"10px 12px",marginBottom:10,display:"flex",alignItems:"center",gap:8}}><Target size={15} color={S.red}/><span style={{fontSize:13,color:S.sub}}>Peso suggerito</span><span style={{fontSize:14,fontWeight:800,color:S.red,marginLeft:"auto"}}>{ex.peso_suggerito} kg</span></div>}
          {/* Log peso */}
          <div style={{background:S.grayLight,borderRadius:10,padding:"10px 12px",marginBottom:10}}>
            <div style={{fontSize:11,color:S.muted,marginBottom:6,fontWeight:600,letterSpacing:"0.5px"}}>IL TUO PESO OGGI</div>
            <div style={{display:"flex",gap:8,alignItems:"center"}}>
              ...
              <button onClick={doLog} style={{background:S.red,border:"none",borderRadius:8,padding:"9px 18px",color:"#FFF",fontWeight:700,cursor:"pointer",fontSize:13,whiteSpace:"nowrap"}}>Salva kg</button>
            </div>
            {logged.length>0&&<div style={{marginTop:8,display:"flex",gap:6,flexWrap:"wrap"}}>{logged.slice(-5).map((l,i)=><span key={i} style={{background:S.white,border:`1px solid ${S.border}`,borderRadius:6,padding:"3px 8px",fontSize:11,color:S.sub}}>{l.date}: <b style={{color:S.text}}>{l.weight}kg</b></span>)}</div>}
          </div>
          {/* Note */}
          {hasNote&&<div style={{background:"#FFF8E1",border:"1px solid #FFE082",borderRadius:10,padding:"10px 12px",marginBottom:10,display:"flex",gap:8}}><AlertCircle size={15} color="#F9A825" style={{flexShrink:0,marginTop:2}}/><span style={{fontSize:13,color:"#5D4037",lineHeight:1.5}}>{ex.note}</span></div>}
          {/* Tecnica */}
          {hasTech&&<button onClick={()=>setShowT(!showT)} style={{width:"100%",background:"none",border:`1px solid ${S.border}`,borderRadius:10,padding:"10px 12px",cursor:"pointer",display:"flex",alignItems:"center",gap:8,marginBottom:10,textAlign:"left"}}>
...
function LoginScreen({config,onLogin,error}){
  ...
  return(
    <div style={{minHeight:"100vh",background:S.bg,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:"32px 24px"}}>
      {/* Logo */}
      <div style={{width:110,height:110,marginBottom:20,borderRadius:"50%",overflow:"hidden",border:"3px solid #E0E0E0",boxShadow:"0 4px 20px rgba(0,0,0,0.12)",background:"#fff"}}>
        <img src={LOGO_URL} alt="Logo" style={{width:"100%",height:"100%",objectFit:"cover"}} onError={e=>{e.target.style.display="none"}}/>
      </div>
      <h1 style={{color:S.text,fontSize:26,fontWeight:900,marginBottom:4,textAlign:"center"}}>{config.nome_palestra||"Master Gym"}</h1>
      {config.slogan&&<p style={{color:S.sub,fontSize:13,marginBottom:36,fontStyle:"italic",textAlign:"center"}}>{config.slogan}</p>}
      <div style={{width:"100%",maxWidth:300}}>
        ...
        <input value={code} onChange={e=>setCode(e.target.value.toUpperCase())} placeholder="es. MG-001" autoComplete="off"
          style={{width:"100%",background:S.white,border:`1.5px solid ${S.border}`,borderRadius:12,padding:"14px 16px",color:S.text,fontSize:16,fontWeight:700,outline:"none",marginBottom:14,boxSizing:"border-box",letterSpacing:"1px"}}
          onFocus={e=>e.target.style.borderColor=S.red} onBlur={e=>e.target.style.borderColor=S.border}/>
        ...
        <input value={pin} onChange={e=>setPin(e.target.value.replace(/\D/g,"").slice(0,4))} placeholder="••••" type="password" inputMode="numeric"
          ...
          style={{width:"100%",background:S.white,border:`1.5px solid ${S.border}`,borderRadius:12,padding:"14px 16px",color:S.text,fontSize:22,fontWeight:700,outline:"none",marginBottom:8,boxSizing:"border-box",letterSpacing:"10px",textAlign:"center"}}
          onFocus={e=>e.target.style.borderColor=S.red} onBlur={e=>e.target.style.borderColor=S.border}/>
        ...
        <button onClick={()=>code.trim()&&pin.trim()&&onLogin(code.trim().toUpperCase(),pin.trim())}
          style={{width:"100%",background:code.trim()&&pin.trim()?S.red:"#BDBDBD",border:"none",borderRadius:12,padding:"15px 16px",color:"#FFF",fontSize:15,fontWeight:800,cursor:code.trim()&&pin.trim()?"pointer":"default",marginTop:8,textTransform:"uppercase",letterSpacing:"1px",transition:"background .2s"}}>
          Accedi
        </button>
      </div>
    </div>
...
function Dashboard({cliente,scheda,esercizi,onSelectDay,onSync,lastSync}){
  ...
  const scaduta=dl<=0;const inScadenza=dl>0&&dl<=7;
  return(
    <div style={{padding:"20px 16px 100px",minHeight:"100vh",background:S.bg}}>
      ...
      <div style={{background:S.white,borderRadius:16,padding:18,marginBottom:20,border:`1px solid ${S.border}`,boxShadow:"0 2px 12px rgba(0,0,0,0.06)"}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:10}}>
          ...
          {scaduta&&<span style={{background:"#FFEBEE",color:S.red,fontSize:11,fontWeight:700,padding:"4px 10px",borderRadius:8,flexShrink:0}}>Scaduta</span>}
        ...
        <button key={sed.nome} onClick={()=>onSelectDay(sed.nome)}
          ...
          <div style={{flex:1,minWidth:0}}>
            <div style={{color:S.text,fontSize:14,fontWeight:700,display:"flex",alignItems:"center",gap:8,flexWrap:"wrap"}}>
              {sed.nome}
              {sed.tipo&&<span style={{background:S.redLight,color:S.red,fontSize:9,fontWeight:800,padding:"2px 7px",borderRadius:4,letterSpacing:"0.5px"}}>{sed.tipo}</span>}
            </div>
...
:S.redLight,color:S.red,fontSize:9,fontWeight:800,padding:"2px 7px",borderRadius:4}}>{tipo}</span>}
          </h1>
...
function ProgressView({progressData,esercizi,schedaId}){
  ...
  const todayLogs=useMemo(()=>{
    const t=new Date().toLocaleDateString("it-IT");const l=[];
  ...
  return(
    <div style={{padding:"20px 16px 100px",minHeight:"100vh",background:S.bg}}>
      ...
      {/* Oggi */}
      <div style={{background:S.white,borderRadius:14,padding:16,marginBottom:20,border:`1px solid ${S.border}`,boxShadow:"0 1px 6px rgba(0,0,0,0.04)"}}>
      ...
      <div style={{display:"flex",flexWrap:"wrap",gap:7,marginBottom:16}}>
        {activeEx.map(ex=>(
          <button key={ex} onClick={()=>setSel(sel===ex?null:ex)}
            style={{background:sel===ex?S.red:S.white,border:`1.5px solid ${sel===ex?S.red:S.border}`,borderRadius:20,padding:"6px 14px",cursor:"pointer",color:sel===ex?"#FFF":S.sub,fontSize:12,fontWeight:600,transition:"all .15s"}}>
            {ex}
          </button>
        ))}
      </div>
      {sel&&(
        <div style={{background:S.white,borderRadius:14,padding:16,border:`1px solid ${S.border}`,boxShadow:"0 1px 6px rgba(0,0,0,0.04)"}}>
          ...
          {selData.length===0
            ...
            :<>
              ...
              {selData.length>=2&&(()=>{
                ...
                return<div style={{display:"flex",alignItems:"center",gap:6,color:diff>=0?"#388E3C":"#D32F2F",background:diff>=0?"#E8F5E9":"#FFEBEE",padding:"8px 12px",borderRadius:8}}><TrendingUp size={15}/><span style={{fontSize:13,fontWeight:700}}>{diff>=0?"+":""}{diff.toFixed(1)} kg dal primo allenamento</span></div>;
              })()}
            </>
          }
        </div>
  ...
  return(
    <div style={{padding:"20px 16px 100px",minHeight:"100vh",background:S.bg}}>
      ...
      {list.map(sc=>{
        ...
        return(
          <div key={sc.scheda_id} style={{background:S.white,borderRadius:14,marginBottom:10,border:`1.5px solid ${active?S.red:S.border}`,overflow:"hidden",boxShadow:"0 1px 6px rgba(0,0,0,0.04)"}}>
                ...
                {seds.map(sed=>{
                  ...
                  return(
                    <div key={sed} style={{marginBottom:12}}>
                      <div style={{fontSize:11,fontWeight:800,color:S.red,marginBottom:6,letterSpacing:"0.5px"}}>{sed}{dayExs[0]?.tipo_seduta?` · ${dayExs[0].tipo_seduta}`:""}</div>
                      {dayExs.map((e,i)=>(
...
// PROFILO
function ProfileView({cliente,config,onLogout}){
  return(
    <div style={{padding:"20px 16px 100px",minHeight:"100vh",background:S.bg}}>
      <h1 style={{color:S.text,fontSize:22,fontWeight:900,marginBottom:20}}>Profilo</h1>
      {/* Cliente */}
      <div style={{background:S.white,borderRadius:16,padding:20,marginBottom:12,border:`1px solid ${S.border}`,boxShadow:"0 1px 6px rgba(0,0,0,0.04)"}}>
      ...
      <div style={{background:S.white,borderRadius:16,padding:20,marginBottom:12,border:`1px solid ${S.border}`,boxShadow:"0 1px 6px rgba(0,0,0,0.04)"}}>
        ...
        <div style={{display:"flex",alignItems:"center",gap:14,marginBottom:14}}>
          <div style={{width:48,height:48,borderRadius:"50%",overflow:"hidden",border:`2px solid ${S.border}`,flexShrink:0}}>
            <img src={LOGO_URL} alt="Logo" style={{width:"100%",height:"100%",objectFit:"cover"}}/>
          </div>
          <div style={{fontSize:16,fontWeight:800,color:S.text}}>{config.nome_palestra||"Master Gym"}</div>
        </div>
        {[config.indirizzo&&[MapPin,config.indirizzo],config.telefono&&[Phone,config.telefono],config.instagram&&[Instagram,config.instagram]].filter(Boolean).map(([I,v],i)=>(
          <div key={i} style={{display:"flex",alignItems:"center",gap:10,marginBottom:8}}>
            <I size={15} color={S.gray}/><span style={{color:S.sub,fontSize:13}}>{v}</span>
          </div>
        ))}
        <div style={{display:"flex",gap:8,marginTop:14}}>
          {config.telefono&&<a href={`tel:${config.telefono}`} style={{flex:1,background:S.redLight,border:`1px solid ${S.redBorder}`,borderRadius:10,padding:"11px 8px",textDecoration:"none",display:"flex",alignItems:"center",justifyContent:"center",gap:6,color:S.red,fontWeight:700,fontSize:13}}><Phone size={15}/> Chiama</a>}
          {config.instagram&&<a href={`https://instagram.com/${config.instagram.replace("@","")}`} target="_blank" rel="noreferrer" style={{flex:1,background:S.grayLight,border:`1px solid ${S.border}`,borderRadius:10,padding:"11px 8px",textDecoration:"none",display:"flex",alignItems:"center",justifyContent:"center",gap:6,color:S.grayDark,fontWeight:700,fontSize:13}}><Instagram size={15}/> Instagram</a>}
        </div>
      </div>
      {/* Logout */}
...
export default function GymApp(){
  ...
  const load=useCallback(async()=>{
    ...
    try{const d=await fetchAllData();setData(d);setLastSync(new Date().toLocaleTimeString("it-IT",{hour:"2-digit",minute:"2-digit"}))}
  ...
  useEffect(()=>{
    ...
    try{
      ...
      if(code){const c=data.clienti.find(cl=>cl.codice===code);if(c&&(!c.pin||String(c.pin).trim()===String(pin).trim())){setCliente(c);setLoggedIn(true)}}
    }catch(e){}
  },[data,loggedIn]);

  const handleLogout=useCallback(()=>{
    setLoggedIn(false);setCliente(null);setTab("home");setSelDay(null);
        ...
        {selDay
          ...
          :tab==="home"&&!scheda
            ?<div style={{padding:"60px 24px",textAlign:"center"}}>
                ...
                <p style={{color:S.muted,fontSize:14}}>Contatta il tuo trainer per assegnare una scheda.</p>
              </div>
          :tab==="progress"
          ...
          :tab==="profile"
            ?<ProfileView cliente={cliente} config={data.config} onLogout={handleLogout}/>
          :null
        }
      </div>
